import { Customer, Campaign, LoyaltyRule, ActivityLog, LoyaltyTier, ChurnRisk, Channel } from './types';
...
const INITIAL_LOYALTY_RULES: LoyaltyRule[] = [
  {
    id: 'RULE-01',
...
let loyaltyRules: LoyaltyRule[] = JSON.parse(JSON.stringify(INITIAL_LOYALTY_RULES));
let activityLogs: ActivityLog[] = JSON.parse(JSON.stringify(INITIAL_ACTIVITY_LOGS));
...
export const db = {
  ...
  addCustomer: (cust: Omit<Customer, 'id' | 'joinedDate'>) => {
  ...
  updateCustomer: (id: string, updates: Partial<Customer>) => {
    ...
    const calculatedTier = calculateTier(updatedPoints);
  ...
  getCampaignById: (id: string) => campaigns.find(c => c.id === id),
  createCampaign: (campaign: Omit<Campaign, 'id' | 'createdDate' | 'sentCount' | 'openCount' | 'clickCount' | 'conversionCount' | 'revenueGenerated'>) => {
  ...
  updateLoyaltyRule: (id: string, updates: Partial<LoyaltyRule>) => {
