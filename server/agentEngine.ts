import { AgentPlan, AgentStep, Campaign, Customer, Channel } from './types';
...
export const generateAgentPlan = (campaignGoal: string): AgentPlan => {
  ...
  let bestChannel: Channel = 'WhatsApp';
  let maxCount = 0;
...
export const runCampaignSimulation = (campaign: Campaign, targetCustomers: Customer[]): {
  sent: number;
  opened: number;
  clicked: number;
  converted: number;
  revenue: number;
  activityLogs: Omit<any, 'id' | 'timestamp'>[];
} => {
